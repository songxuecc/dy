  ...
  <div style="height: 100%">
    <el-row :gutter="20" style="height: 100%">
      <el-col :span="8" style="height: 100%; padding-right: 0px; padding-bottom: 80px;">
        <el-table ref="productList" :data="productList" row-key="tp_product_id" border :show-header="false" :cell-style="cellStyle"
...
export default {
  ...
  methods: {
    ...
    handleProductSelect (val, old) {
      if (old) {
        if (this.product.isDiff() || this.attrApplyCatMap[this.product.model.cat_id]) {
          old.isEdit = true
        } else {
          old.isEdit = false
        }
    ...
    updateAttrApplyCat (attrApplyCat) {
      for (let i in this.productList) {
        ...
        if ((this.products[tpProductId] && this.products[tpProductId].isDiff()) || attrApplyCat[this.productList[i].category_id]) {
          this.productList[i].isEdit = true
        } else {
    ...
    onSaveProduct () {
      ...
      this.requestBatchUpdateTPProduct(tpProductList, tpProductIdList, this.attrApplyCatMap, 0, 0)
    },
    requestBatchUpdateTPProduct (tpProductList, tpProductIdList, attrApplyCatMap, tpProductListIdx, tpProductIdListIdx) {
      ...
      this.request('batchUpdateTPProduct', {
        ...
        attr_apply_map: JSON.stringify(attrApplyCatMapTemp)}, data => {
        // 更新商品列表信息
        ...
        if (tpProductListIdx >= tpProductList.length && tpProductIdListIdx >= tpProductIdList.length) {
          ...
          if (tpProductListIdx < tpProductList.length) {
            self.requestBatchUpdateTPProduct(tpProductList, tpProductIdList, attrApplyCatMap, tpProductListIdx + 5, tpProductIdListIdx)
          } else {
            self.requestBatchUpdateTPProduct(tpProductList, tpProductIdList, attrApplyCatMap, tpProductListIdx, tpProductIdListIdx + 5)
          }
        }
      }, data => {
        this.isProductEditSaving = false
      ...
      for (let i in data) {
        ...
        if (tpProductId in this.products) {
          ...
          let tpProduct = {
            ...
            title: this.productDic[tpProductId].title,
    ...
    isProductChange () {
      ...
      for (let i in this.productList) {
        let tpProductId = this.productList[i].tp_product_id
        if ((this.products[tpProductId] && this.products[tpProductId].isDiff()) || this.attrApplyCatMap[this.productList[i].category_id]) {
          isChanged = true
        }
      }
      return isChanged
