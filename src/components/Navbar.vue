            ...
            <div class="nav-right">
                 ...
                 <el-tooltip v-if="subsc.order_list.length">
                  <div slot="content" >
                    <p v-for="order in subsc.order_list" :key="order.order_id"  class="left font-12">
                      {{order.start_time}}~{{order.end_time}} 您的版本为 {{order.sku_spec || '-'}} <a v-if="!order.is_free_upgrate " class="primary pointer" @click="paidUp">{{versionType === 'free_three_months'?'升级':'订购'}}高级版</a>
                    </p>
                  </div>
              ...
              <el-tooltip v-if="shopName" class="syncProductToolTip" effect="dark" placement="bottom">
                <template slot="content">
                  <p style="font-size: 12px;margin-bottom:2px">将抖店后台的商品立马同步至本软件</p>
...
export default {
  ...
  methods: {
    ...
    async paidUp () {
      if (this.versionType === 'free_three_months') {
        if (window._hmt) {
