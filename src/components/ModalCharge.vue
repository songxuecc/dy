<template>
  <el-dialog
    :visible.sync="modalChargeVisible"
    append-to-body
    width="500px"
    class="ModalCharge"
    :show-close="false"
  >
    <div class="ModalVersionUp " >
      ...
      <div class="tip mb-10" v-else>非抖音平台受复制成本影响有额度限制，当前剩余{{modalChargeData.left_capture_nums}}条，请充值</div>
...
export default {
  ...
  computed: {
    ...mapState('migrate/readyToMigrate', ['userVersion', 'versionType', 'modalChargeData', 'modalChargeVisible']),
  ...
  methods: {
    ...
    open () {
      this.save({
        modalChargeVisible: !this.modalChargeVisible
      })
    },
    close () {
      this.save({
