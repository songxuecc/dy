  ...
  actions: {
    ...
    formatTableData ({commit, state, rootGetters}, payload) {
      ...
      const nextTableData = (tableData || []).map(item => {
        ...
        } else if (item.selectPriceType === 1) {
        ...
        if (singleSkuPrice) {
          ...
          if (item.tp_product_id === id && Number(arithmetic.radio) === 2) {
            ...
            item.group_price_range = item.currentMinPrice
            item.selectPriceInfo = `统一价格为 ${item.currentMinPrice}`
            ...
            if (singleGroupPriceRange !== item.group_price_range) {
              ...
              // 重设sku价格后 需要重新设置售卖价的范围
              item.discount_price = !Number(template.model.is_sale_price_show_max) ? evalSingleSkuGroupPriceRange(minPrices) : evalSingleSkuGroupPriceRange(maxPrices)
            } else {
