...
const model = modelExtend(tableDataRecord, tableDataDetail, {
  ...
  actions: {
    async fetchRecord ({commit, state}, payload) {
      await this.dispatch('productManagement/skuImport/productSkuExcelFetch', {
    ...
    fetchDetail ({commit, state}, payload) {
      this.dispatch('productManagement/skuImport/skuExcelDetailFetch', {
