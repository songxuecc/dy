  ...
  <div style="position: relative;">
    ...
    <el-tabs v-model="activeName">
      ...
      >
        ...
        <el-radio-group
          v-model="binCopyActiveName"
          size="small"
          class="mb-10 bindCopyRadio"
          v-if="userBindList.length"
          @change="handleBinCopyActiveNameChange"
        >
        ...
        <el-form
          :inline="true"
          :model="modelBindCopy"
          class="start-migrate-setting flex wrap"
          size="medium"
          ref="modelCopyForm"
          :rules="modelBindCopyRules"
          v-if="userBindList.length"
        >
          <el-form-item
            label="被复制的店铺"
          ...
          >
            ...
            <el-button type="text" @click="gotoBindShop" size="small">
              绑定新店铺
            </el-button>
            <div
              class="info"
              v-if="target_user_id"
              style="position:absolute;left:0;bottom:-12px;width:500px;transform: translateY(100%);"
            >
              <div class="font-12">
                <div
                  class="font-12"
                  style="width:350px;word-break:break-all;line-height:18px"
                >
                  {{ bandShopTip.shop_name }}&nbsp;最近更新时间{{
                    bandShopTip.last_goods_sync_time
                  }}
                  <p
                    v-if="!syncText"
                    v-loading="syncLoading"
                    @click="handleSyncProducts(target_user_id)"
                  >
                    <span class="fail">
                      <hh-icon type="iconjingshi1"> </hh-icon>
                      复制前需进行店铺数据同步，
                    </span>
                    <span class="primary">点击开始同步</span>
                  </p>
                  <p
                    class="primary"
                    v-if="syncText"
                    v-loading="syncLoading"
                    :style="syncText && syncText.includes('同步中') ? '' : ''"
                  >
                    {{ syncText }}
                    <span v-if="syncText === '同步中'">
          ...
          <el-form-item
            label="状态选择"
            v-show="binCopyActiveName === 'status'"
          >
            <el-select
              v-model="modelBindCopy.status"
              placeholder="商品状态选择"
              style="width:257px;margin-right:30px"
            >
          ...
          <el-form-item label="是否过滤已复制商品">
            <el-select
              v-model="modelBindCopy.filter_copied_product"
              placeholder="商品状态选择"
              style="width:263px;"
            >
    ...
    <div class="startCopyBtn flex align-c" v-if="activeName === 'file'">
      <div style="height:50px">
        <el-button
          type="primary"
          @click="gotoSettingFile(onCaptureBindCopy, true)"
          style="height:50px;font-size:16px"
          class="ralative"
          :disabled="
          ...
          >下一步：复制设置
          <span
            v-if="productListCheckLoading"
            class="info"
            style="position:absolute;right:-114px;top:12px"
            >正在查询，请稍后...
          </span>
        </el-button>
        <el-popover
    ...
    <!-- 绑定复制 -->
    <div
      class="startCopyBtn flex align-c"
      v-if="activeName === 'bindCopy' && userBindList.length"
    >
      <div style="height:50px">
        <el-button
          type="primary"
          @click="gotoSetting(onCaptureBindCopy, true)"
          style="height:50px;font-size:16px"
          class="ralative"
          :disabled="
            isStartCapture || settingDataLoading || productListCheckLoading
    ...
    >
      <iframe
        v-bind:src="
          'https://view.officeapps.live.com/op/embed.aspx?wdAccPdf=1&ui=zh-cn&rs=zh-cn&src=https://hhgj-manual.oss-cn-shanghai.aliyuncs.com/怎么安装及使用虎虎复制助手插件？.docx'
        "
        width="100%"
        height="800px;"
        frameborder="0"
...
const { mapActions: mapActionsPaidRecharge } = createNamespacedHelpers(
...
export default {
  ...
  watch: {
    ...
    target_user_id (newVal) {
      if (newVal) {
        ...
        const bandShopTip = this.userBindList.find(
          item => this.target_user_id === item.user_id
        )
  ...
  methods: {
    ...mapActions(['setCaptureIdList']),
    ...mapActions('migrate/startMigrate', ['getCaptureShopCompleteList']),
    ...mapActionsPaidRecharge([
      'getUserAccountQuery',
      'availablePddCaptureNums'
    ]),
    ...
    async getUserBindList () {
      ...
      arr.forEach(item => {
        if (!bandShopsMap.get(item.user_id)) {
          ...
          item.disabled =
                item.auth_status === 'expire' || item.auth_status === 'un_login'
          item.shop_name =
                item.auth_status === 'expire'
                  ? `${item.shop_name}(订购过期)`
                  : item.shop_name
          item.shop_name =
                item.auth_status === 'un_login'
                  ? `${item.shop_name}(登录过期)`
                  : item.shop_name
    ...
    onCaptureShops () {
      ...
      this.isCaptureIsExistShopCapture = true

      // 查询是否重复
      this.request(
        'captureIsExistShopCapture',
        {
          urls: JSON.stringify(urls)
        },
        data => {
          this.isCaptureIsExistShopCapture = false
                ...
                h(
                  ...
                  [
                    h(
                      'div',
                      null,
                      `该链接于 ${data.capture_detail.create_time} 被您复制过。您希望`
                    ),
    ...
    async onCaptureBindCopy () {
      ...
      try {
        ...
        if (this.binCopyActiveName === 'id') {
          ...
          const goodsIds = this.modelBindCopy.goods_ids
            .split(/[\s\n]/)
            .filter(item => item)
            .map(item => item.trim())
          const goodsIdsSet = [...new Set(goodsIds)]
          ...
          if (idsCheck && idsCheck.lost_goods_id_list.length) {
            ...
            const unionSets = goodsIdsSet.filter(
              item => !lostGoodsIdsSet.has(item)
            )
            if (unionSets.length) {
              // 弹出弹窗
    ...
    uploadOnSuccess: async function (response, file, fileList) {
      ...
      if (
        !isFreeUpgrate &&
        !isSenior &&
        response.data.left_capture_nums_not_enough
      ) {
        // 3个月试用引导内部升级
        // 7天试用引导在服务市场
        if (versionTipType === 'free_three_months') {
