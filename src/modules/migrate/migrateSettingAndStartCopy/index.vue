<template lang="html">
  <div class="migrateSetting relative">
    <div :style="{width: `calc(100% - ${scrollWidth + 282}px)`}" class="titlea">
      <Title />
      <el-tabs tab-position="top"  v-model="activeTab"  @tab-click="tabClick" ref="tab"  class="tab">
    ...
    <div :style="{'text-align': 'left', 'font-size': '14px','padding-bottom': mBottom,'padding-top': '120px'}" class="migrateSettingForm">
...
<!--        </el-form-item>-->
        ...
        <el-form-item  label="属性设置:"  style="padding-bottom: 20px;box-sizing: border-box" class="flex migrateSetting-attribute" v-loading="loadingCategoryMap">
          <div style="margin-bottom:20px" class="align-c" v-loading="loadingPropertiesMap">
            ...
            <div class="font-12 mb-5" v-for="(properties,idx) in showPropertiesMap" :key="properties.id" v-if="propertiesMap.length">
              当属性维度是
              <el-input size="mini" style="width:150px" @change="changeProperties" class="ml-5 mr-5" :placeholder="idx !==0 ? '请填写,如产地':'请填写,如品牌'" v-model="properties.name"></el-input> 时，对应的属性值是
              <el-input  size="mini"  style="width:150px"  class="ml-5 mr-5" :placeholder="idx !==0?'请填写,如中国大陆':'请填写,如雅诗兰黛'" v-model="properties.value"></el-input>
        ...
        <el-form-item  label="重复搬家设置"  class="flex migrateProductsFilter migrateSetting-choose" style="padding-bottom: 20px;box-sizing: border-box">
        ...
        <el-form-item  label="违规信息:"  style="padding-bottom: 20px;box-sizing: border-box" class="flex migrateSetting-rule" >
            <div>
              <span style="font-size: 12px;">商品标题、SKU规格名违规词自动删除</span>
...
export default {
  ...
  computed: {
    ...
    ...mapState('migrate/migrateSettingAndStartCopy', ['captureCallBack']),
    getImageFirstData () {
      return {
